...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if surname:
        surname_analysis = f"""
    Фамилия: {surname}
    Происхождение: укажите предположительное происхождение (топоним, имя, профессия)
    Национальность: укажите (русская, татарская, еврейская и т.д.)
    Географическое распространение: укажите регионы (например: Архангельская, Мурманская области)
    Суффикс: укажите значение (-ский, -ов, -ин, -ко)
    Варианты написания: перечислите (например: Барминский, Бармин, Барминов)
    Связь с профессией/именем/местом: укажите (например: от имени Барма)
    """
    else:
        surname_analysis = "Фамилия не указана."
    # 🔼
...
"""
    try:
        ...
        completion = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты — профессиональный генеалог с 20 летним опытом исследований родословных. Пиши чётко, по делу, с источниками, тарифами и ссылками. Не добавляй вступления вроде 'Вот пошаговая стратегия' — начинай сразу с содержания. Никогда не давай общих советов вроде 'проведите исследование'."},
